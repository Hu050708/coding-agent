"""Synchronous ClearLoop runner used inside the Web service worker pool."""
...
from clearloop.core import DEFAULT_SYSTEM_PROMPT, Agent, AgentConfig, RunResult, TraceEmitter
from clearloop.diagnostics import TraceWriter
from clearloop.providers import DeepSeekAdapter
from clearloop.security import CommandRequest, Workspace
from clearloop.tools import ToolRegistry

from clearloop.config import AppSettings
from clearloop.memory import MemoryPromptBuilder, MemoryService, MemorySummary
    ...
    def run(
        ...
        *,
        ...
        if self.settings.trace_enabled:
            trace_path = workspace.root / ".clearloop-traces" / f"web-{spec.run_id}.jsonl"
