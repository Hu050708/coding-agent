...
from clearloop.security.workspace import Workspace, WorkspaceError
...
def list_files(workspace: Workspace, arguments: Mapping[str, Any]) -> dict[str, Any]:
    ...
    while stack and not truncated and not scan_limit_reached:
        ...
        # LIFO stack: reverse to visit directories in the same stable order in
...
def replace_text(
    ...
    *,
    ...
    if expected_matches != 1:  # Kept explicit as a protocol invariant.
...
def _normalize_replacement_newlines(text: str, source_style: str) -> str:
    """Match a replacement to a source file's single newline convention.

    Mixed-newline and newline-free source files are left alone: there is no
    unambiguous convention to impose in either case.
    """
