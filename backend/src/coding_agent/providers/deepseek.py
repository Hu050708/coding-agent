"""DeepSeek Chat Completions adapter using the official OpenAI Python client.

The SDK is used only for HTTP transport and response data objects.  This module
does not parse tool arguments, execute tools, manage history, or run an agent
loop; those responsibilities remain in :mod:`coding_agent.core.agent`.
"""
...
def normalize_completion(raw_response: Any) -> ModelCompletion:
    """Convert one SDK response into the provider-independent core contract."""
    ...
    # These responses never enter history.  In particular, a truncated partial
    # tool call must not be parsed or accidentally executed.
    if finish_reason in _NON_HISTORICAL_FINISH_REASONS:
        ...
        # Preserve the reason so the core decision table can classify it, while
        # deliberately discarding an unrecognized message shape.
        assistant = AssistantMessage()
...
class DeepSeekAdapter:
    """One-request-at-a-time DeepSeek Chat Completions transport."""
    ...
    def close(self) -> None:
        """Best-effort, idempotent release of the underlying SDK client.

        A client without a callable ``close`` needs no adapter-managed cleanup.
        Cleanup failures are deliberately suppressed so a ``finally`` block
        cannot hide the run result or expose exception text containing request
        or credential data.  Once attempted, cleanup is not invoked again.
        """
