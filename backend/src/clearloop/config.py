"""Server-only configuration for the local ClearLoop Web UI."""
...
def _default_data_dir() -> Path:
    ...
    if local_app_data:
        return Path(local_app_data) / "ClearLoop"
    return Path.home() / ".local" / "share" / "ClearLoop"
...
class AppSettings:
    ...
    def __post_init__(self) -> None:
        ...
        if not root.is_absolute():
            raise SettingsError("CLEARLOOP_ALLOWED_ROOT must be an absolute path.")
        try:
            root = root.resolve(strict=True)
        except (OSError, RuntimeError) as exc:
            raise SettingsError("CLEARLOOP_ALLOWED_ROOT must be an existing directory.") from exc
        if not root.is_dir():
            raise SettingsError("CLEARLOOP_ALLOWED_ROOT must be a directory.")
        object.__setattr__(self, "allowed_root", root)

        data_dir = Path(self.data_dir).expanduser()
        if not data_dir.is_absolute():
            raise SettingsError("CLEARLOOP_DATA_DIR must be an absolute path.")
        try:
            data_dir = data_dir.resolve(strict=False)
        except (OSError, RuntimeError) as exc:
            raise SettingsError("CLEARLOOP_DATA_DIR is invalid.") from exc
        try:
        ...
        if os.path.normcase(common) == os.path.normcase(os.fspath(root)):
            raise SettingsError(
                "CLEARLOOP_DATA_DIR must be outside CLEARLOOP_ALLOWED_ROOT."
            )
        object.__setattr__(self, "data_dir", data_dir)

        if self.host != "127.0.0.1":
            raise SettingsError("ClearLoop Web must bind to 127.0.0.1.")
    ...
    def from_environment(
        ...
        *,
        ...
        allowed_root = Path(get("CLEARLOOP_ALLOWED_ROOT") or DEFAULT_ALLOWED_ROOT)
        return cls(
            api_key=(get("DEEPSEEK_API_KEY") or "").strip(),
            allowed_root=allowed_root,
            data_dir=Path(get("CLEARLOOP_DATA_DIR") or _default_data_dir()),
            model=(get("CLEARLOOP_MODEL") or DEFAULT_MODEL).strip(),
            base_url=(get("CLEARLOOP_BASE_URL") or DEFAULT_BASE_URL).strip(),
            max_tokens=_parse_int(
                "CLEARLOOP_MAX_TOKENS", get("CLEARLOOP_MAX_TOKENS"), DEFAULT_MAX_TOKENS,
                minimum=1, maximum=1_000_000,
            ),
            max_active_runs=_parse_int(
                "CLEARLOOP_MAX_ACTIVE_RUNS", get("CLEARLOOP_MAX_ACTIVE_RUNS"), 1,
                minimum=1, maximum=4,
            ),
            max_retained_runs=_parse_int(
                "CLEARLOOP_MAX_RETAINED_RUNS", get("CLEARLOOP_MAX_RETAINED_RUNS"), 50,
                minimum=1, maximum=1_000,
            ),
            event_buffer_size=_parse_int(
                "CLEARLOOP_EVENT_BUFFER_SIZE", get("CLEARLOOP_EVENT_BUFFER_SIZE"), 256,
                minimum=32, maximum=10_000,
            ),
            approval_timeout_seconds=_parse_float(
                "CLEARLOOP_APPROVAL_TIMEOUT_SECONDS", get("CLEARLOOP_APPROVAL_TIMEOUT_SECONDS"), 480.0,
                minimum=1.0, maximum=86_400.0,
            ),
            max_model_calls=_parse_int(
                "CLEARLOOP_MAX_MODEL_CALLS", get("CLEARLOOP_MAX_MODEL_CALLS"), 16,
                minimum=1, maximum=1_000,
            ),
            max_tool_calls=_parse_int(
                "CLEARLOOP_MAX_TOOL_CALLS", get("CLEARLOOP_MAX_TOOL_CALLS"), 40,
                minimum=1, maximum=10_000,
            ),
            max_total_tokens=_parse_int(
                "CLEARLOOP_MAX_TOTAL_TOKENS", get("CLEARLOOP_MAX_TOTAL_TOKENS"), 200_000,
                minimum=1, maximum=10_000_000,
            ),
            wall_time_seconds=_parse_float(
                "CLEARLOOP_WALL_TIME_SECONDS", get("CLEARLOOP_WALL_TIME_SECONDS"), 480.0,
                minimum=1.0, maximum=86_400.0,
            ),
            api_timeout_seconds=_parse_float(
                "CLEARLOOP_API_TIMEOUT_SECONDS", get("CLEARLOOP_API_TIMEOUT_SECONDS"),
                DEFAULT_TIMEOUT_SECONDS, minimum=1.0, maximum=600.0,
            ),
            max_transient_retries=_parse_int(
                "CLEARLOOP_MAX_TRANSIENT_RETRIES", get("CLEARLOOP_MAX_TRANSIENT_RETRIES"), 3,
                minimum=0, maximum=20,
            ),
            trace_enabled=_parse_bool(
                "CLEARLOOP_TRACE_ENABLED", get("CLEARLOOP_TRACE_ENABLED"), True
            ),
            port=_parse_int(
                "CLEARLOOP_WEB_PORT", get("CLEARLOOP_WEB_PORT"), 8000,
